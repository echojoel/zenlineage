/**
 * Seed European Sōtō disciples of Taisen Deshimaru that are not already
 * present in the reconciled canonical dataset. Mirrors the structure of
 * scripts/seed-maezumi-lineage.ts so the AZI / Deshimaru neighborhood is
 * built from a single, citable data file.
 *
 * Idempotent — safe to re-run.
 *
 * Usage:
 *   npm run seed:deshimaru
 */
...
import { DESHIMARU_DISCIPLES } from "./data/deshimaru-lineage";
...
async function main() {
  console.log(
    `Seeding Deshimaru lineage (${DESHIMARU_DISCIPLES.length} European disciples)...\n`
  );

  for (const m of DESHIMARU_DISCIPLES) {
  ...
  console.log(`✓ ${DESHIMARU_DISCIPLES.length} Deshimaru disciples upserted`);
