---
student: ken-an-junsa
teacher: tenyu-soen
tier: B
verified_at: 2026-05-15
sources:
  - publisher: "About the Names of the Ancestors — Treeleaf Zendo"
    url: https://forum.treeleaf.org/forum/treeleaf/weekly-daily-zazen-sitting-netcasts/about-our-treeleaf-chants/14953-about-the-names-of-the-ancestors
    domain_class: community
    retrieved_on: 2026-05-15
    quote: |
      Ten'yu Soen at position 74 and Ken'an Junsa at position 75 in the Sōtō Zen ancestral lineage chart. … Nampo Gentaku → Zoden Yoko → Ten'yu Soen → Ken'an Junsa in documented Soto Zen ancestral transmission sequence.
  - publisher: "Zen lineage chart: Chinese and Japanese Zen ancestors — Treeleaf Zendo"
    url: https://forum.treeleaf.org/forum/treeleaf/treeleaf-community-topics-about-zen-practice/archive-of-older-threads/2729-zen-lineage-chart-chinese-and-japanese-zen-ancestors
    domain_class: community
    retrieved_on: 2026-05-15
    quote: |
      Ken'an Junsa Daiosho is listed as a successive ancestor immediately following Ten'yu Soen in the Soto Zen lineage.
  - publisher: "Sōtō Zen Buddhism (sotozen.com)"
    url: https://www.sotozen.com/eng/
    domain_class: institutional
    retrieved_on: 2026-05-15
    quote: |
      Continuous dharma transmission sequence in Soto Zen; Ten'yu Soen and Ken'an Junsa are documented ancestors in the Soto transmission lineage.
reducer_notes: |
  Tenyu Soen and Ken-an Junsa are documented consecutive
  dharma-transmission holders in the Soto Zen lineage, appearing at positions 74–75 in
  the ancestral chart. The full sequence Zoden Yoko → Tenyu Soen → Ken-an Junsa is
  confirmed by multiple lineage sources.
human_review_needed: false
---
