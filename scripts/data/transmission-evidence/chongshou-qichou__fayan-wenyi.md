---
student: chongshou-qichou
teacher: fayan-wenyi
tier: C
verified_at: 2026-05-15
sources:
  - publisher: "Terebess Asia Online — 法眼文益 Fayan Wenyi (855-958)"
    url: https://terebess.hu/zen/fayan.html
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      Fayan Wenyi "first taught at Chongshou Monastery in Linchuan." He had
      "fourteen heirs who went out into the world, honoured by rulers and
      nobles alike," plus "forty-nine other disciples later made the Dharma
      available, each one propagating it in their respective region."
reducer_notes: |
  C-tier: one credible source. Chongshou Qichou is
  not explicitly named in accessible English-language sources as one of Fayan's
  heirs. The "Chongshou" prefix likely denotes residence at Chongshou Monastery
  in Linchuan where Fayan first taught. Fayan had 63 documented heirs (Jingde
  Chuandeng Lu) but English sources name only Tiantai Deshao and Wensui
  prominently. Tier C: plausible from context but explicit attestation missing.
human_review_needed: true
---
