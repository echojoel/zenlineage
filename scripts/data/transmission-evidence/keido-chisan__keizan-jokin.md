---
student: keido-chisan
teacher: keizan-jokin
tier: C
verified_at: "2026-05-22"
sources:
  - publisher: "Dharma Rain Zen Center – Traditional Line of Dharma Ancestors"
    url: https://dharma-rain.org/about/our-teachers/our-lineage/
    domain_class: institutional
    retrieved_on: "2026-05-15"
    quote: |
      Kei·zan Jo·kin … [through many generations] … Kei·do Chisan — Keido Chisan appears 15 generations after Keizan Jokin in the Dharma Rain ancestral chant (Sojiji / Jiyu Kennett lineage), not as his direct student but as a lineage descendant.
  - publisher: "Keizan Study Material for the 2010 Soto Zen Buddhist Association National Conference – terebess.hu"
    url: https://terebess.hu/zen/Keizan_Study.pdf
    domain_class: academic
    retrieved_on: "2026-05-15"
    quote: |
      The lineage goes through Meihō Sotetsu and Manzan Dohaku to Keidō Chisan who was Abbot of Sōjiji when Rev. Master Jiyu trained there. Yokoji is the temple where Keido Chisan was ordained by Koho Hakugan and trained as a young monk in the 1890s.
  - publisher: "Terebess Asia Online — Jiyu-Kennett lineage chart"
    url: https://terebess.hu/zen/mesterek/jiyukennett.html
    domain_class: reference
    retrieved_on: "2026-05-15"
    quote: |
      孤峰白岩 Kohō Hakugan directly preceding 莹堂智璨 Keidō Chisan (1879-1967) [孤峰禅師 Kohō zenji]; ancestry extending back through 卍山道白 Manzan Dōhaku (1635-1715) and 瑩山紹瑾 Keizan Jōkin (1268-1325) near the beginning of the recorded Soto Zen transmission chain
reducer_notes: |
  IMPORTANT FLAG: This edge represents a LINEAGE ANCESTRY
  connection, not a direct teacher-student transmission. Keizan Jokin (1268–1325) and
  Keido Chisan (1879–1967) are separated by approximately 15 generations and roughly
  600 years. The terebess.hu Jiyu-Kennett lineage chart explicitly shows the full chain:
  Keizan Jōkin → [many generations] → Manzan Dōhaku → [further generations] →
  Kohō Hakugan → Keidō Chisan. Keido Chisan's direct dharma teacher was Kohō Hakugan
  (ordained at Eikoji, Hakui, Ishikawa Prefecture, 1889).

  Three independent sources now document the lineage ancestry connection, but none
  establish a direct transmission. Tier remains C because a direct teacher-student
  edge between these two cannot be supported. The edge in the database should be
  reviewed — if it encodes lineage ancestry rather than direct transmission, it should
  be annotated as such or the edge type changed.

  Additional source confirmed 2026-05-22: Bright Way Zen lineage page
  (brightwayzen.org) identifies Keido Chisan Koho Zenji (1879–1967) as
  "abbot of one of the two head training monasteries of the Soto Zen
  Buddhism sect in Japan," confirming his role at Sojiji. No new source
  narrows the direct transmission teacher — Koho Hakugan remains the
  documented direct teacher.

  Searched sotozen-net.or.jp for Keido Chisan: no directly retrievable
  English-language page found. Tier B/A upgrade blocked by the fundamental
  non-directness of this transmission edge.
human_review_needed: true
---
