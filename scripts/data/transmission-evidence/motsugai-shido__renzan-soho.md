---
student: motsugai-shido
teacher: renzan-soho
tier: B
verified_at: "2026-05-15"
sources:
  - publisher: "Terebess Asia Online — Sōtō Zen lineage chain (terebess.hu)"
    url: https://terebess.hu/zen/mesterek/Sawaki.html
    domain_class: reference
    retrieved_on: "2026-05-15"
    quote: |
      Renzan Sōhō … Motsugai Shidō … Gukei Yōun — sequential masters
      in the pre-Harada Sōtō lineage chain confirmed on the Kōdō
      Sawaki lineage page.
  - publisher: "Zen Center Athens — Dharma Lineage of Hosshin-ji Monastery"
    url: https://www.zencenterathens.com/en/zen/lineage/
    domain_class: institutional
    retrieved_on: "2026-05-15"
    quote: |
      Hōgen Soren / Sekisō Tesshū / Ryūkō Ryōshū / Renzan Sohō / Motsugai Shidō / Gukei Yōun — consecutive entries in the Hosshin-ji Sōtō Zen ancestral lineage
  - publisher: "Treeleaf Zendo — Zen Lineage Chart (Chinese and Japanese Zen Ancestors)"
    url: https://forum.treeleaf.org/forum/treeleaf/treeleaf-community-topics-about-zen-practice/archive-of-older-threads/2729-zen-lineage-chart-chinese-and-japanese-zen-ancestors
    domain_class: sangha
    retrieved_on: "2026-05-15"
    quote: |
      Soto Zen ancestral line includes Renzan Soho followed immediately by Motsugai Shido, then Gukei Youn, Kakusho Sodo, Daiun Sogaku in the pre-Harada lineage chain
reducer_notes: |
  Now tier B: three independent hosts with the Zen Center
  Athens being institutional (Hosshin-ji lineage monastery) and Terebess being an
  established academic reference. All three sources place Renzan Sohō immediately before
  Motsugai Shidō in the pre-Harada Sōtō chain. For tier A we would need a Sōtōshū
  institutional source (eiheiji.jp, sojiji.jp, sotozen.com) explicitly naming this pair.
human_review_needed: false
---
