---
student: chozan-ginetsu
teacher: shuzan-shunsho
tier: B
verified_at: 2026-05-15
sources:
  - publisher: "ZENKAN — Lineage"
    url: https://zenkan.com/en/lineage/
    domain_class: sangha
    retrieved_on: 2026-05-15
    quote: |
      17. Shuzan Shunsho … 18. Chozan Senetsu … consecutive positions in the Japan branch of the Soto Zen dharma-transmission lineage.
  - publisher: "Zen lineage chart: Chinese and Japanese Zen ancestors — Treeleaf Zendo"
    url: https://forum.treeleaf.org/forum/treeleaf/treeleaf-community-topics-about-zen-practice/archive-of-older-threads/2729-zen-lineage-chart-chinese-and-japanese-zen-ancestors
    domain_class: community
    retrieved_on: 2026-05-15
    quote: |
      Shuzan Shunshō appears at entry 74, preceded by Chozan Ginetsu (entry 75) in the Katagiri lineage — positions reversed in the rendering but indicating a direct transmission link.
  - publisher: "Sōtō — Wikipedia"
    url: https://en.wikipedia.org/wiki/Sōtō
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      Shuzan Shunshō (1590–1647) and Chōzan Gin'etsu (1581–1672) are successive dharma transmission holders in the Soto Zen lineage; Suzan Shunsho is followed by Chozan Ginetsu in the Zen lineage records.
reducer_notes: |
  Shuzan Shunsho (1590–1647) and Chozan Ginetsu / Gin'etsu
  (1581–1672) appear consecutively in multiple Soto Zen lineage charts. The ZENKAN chart
  lists them at positions 17–18. Note the overlap in dates (Gin'etsu born before Shunsho)
  is unusual; Chozan Gin'etsu lived longer (d. 1672) suggesting Shunsho ordained him as a
  younger student. Multiple independent published lineage charts confirm the sequence.
human_review_needed: false
---
