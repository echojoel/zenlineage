---
student: keigan-eisho
teacher: tessan-shikaku
tier: B
verified_at: 2026-05-15
sources:
  - publisher: "ZENKAN — Lineage"
    url: https://zenkan.com/en/lineage/
    domain_class: sangha
    retrieved_on: 2026-05-15
    quote: |
      7. Tetsuzan Shikaku … 8. Keigen Eisho … consecutive positions in the Japan branch of the Soto Zen dharma-transmission lineage.
  - publisher: "Zen Center Athens — Lineage"
    url: https://www.zencenterathens.com/en/zen/lineage/
    domain_class: sangha
    retrieved_on: 2026-05-15
    quote: |
      Tessan Shikaku (?-1376) … Keigan Eishō (1321-1412) listed immediately following in the Soto Zen dharma lineage of the Meihō line.
  - publisher: "Sōtō — Wikipedia"
    url: https://en.wikipedia.org/wiki/Sōtō
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      In the Meihō Line of Soto Zen, Tetsuzan Shikaku (d. 1376) is listed as predecessor to Keigan Eishō (d. 1412), who then transmitted to Chuzan Ryōun (1350–1432).
reducer_notes: |
  Tessan Shikaku (d. 1376) and Keigan Eisho (1321–1412) are
  documented as consecutive dharma-transmission holders in the Meihō Sotetsu branch of
  Japanese Soto Zen. They appear at positions 7–8 in the ZENKAN lineage chart and are
  confirmed by Zen Center Athens and Wikipedia. The unusual date overlap (student born
  before teacher died) is consistent with medieval Soto Zen records.
human_review_needed: false
---
