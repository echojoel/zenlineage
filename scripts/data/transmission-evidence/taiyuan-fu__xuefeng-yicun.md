---
student: taiyuan-fu
teacher: xuefeng-yicun
tier: B
verified_at: "2026-05-15"
sources:
  - publisher: "Wikipedia (Xuefeng Yicun)"
    url: https://en.wikipedia.org/wiki/Xuefeng_Yicun
    domain_class: reference
    retrieved_on: "2026-05-15"
    quote: |
      Xuefeng Yicun's heirs included Xuansha (835-908), Changqing (854-932), Yunmen (864-949), Baofu (868?-928), Cuiyan, Jingqing (868-937), and Taiyuan Fu.
  - publisher: "Terebess — Master List"
    url: https://terebess.hu/english/master.html
    domain_class: reference
    retrieved_on: "2026-05-15"
    quote: |
      Taiyuan Fu was a dharma heir of Xuefeng Yicun. The tenzo-enlightenment story under Xuefeng is recorded in lamp records.
  - publisher: "White Wind Zen Community — Master List of Masters"
    url: https://wwzc.org/master-list-masters/
    domain_class: sangha
    retrieved_on: "2026-05-15"
    quote: |
      Disciple of Xuefeng awakened by a tenzo. Dogen refers to this story in Tenzo kyokun; see Cooking Zen.
reducer_notes: |
  Three independent sources confirm Xuefeng Yicun
  as Taiyuan Fu's dharma teacher: Wikipedia listing Taiyuan Fu explicitly among
  Xuefeng's heirs, Terebess Master List, and WWZC. The Dōgen reference to the
  tenzo-awakening story (Tenzo kyokun) provides additional institutional
  corroboration. Upgraded from C to B: two independent hosts
  (en.wikipedia.org, terebess.hu) plus institutional sangha source (wwzc.org).
human_review_needed: false
---
