---
student: taizan-maezumi
teacher: yamada-koun
tier: B
verified_at: 2026-05-15
sources:
  - publisher: "Yamada Koun — Wikipedia"
    url: https://en.wikipedia.org/wiki/Yamada_Koun
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      山田耕雲 [Yamada Kōun (Zenshin)] (1907–1989) with 前角 (大山) 博雄 [Maezumi (Taizan) Hakuyū] listed as a dharma heir in the Terebess lineage chart citing this transmission.
  - publisher: "The Two Main Lineages of Modern Sōtō — Terebess"
    url: https://terebess.hu/zen/mesterek/lineage.html
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      Yamada Kōun (Zenshin) (1907–1989) with Maezumi (Taizan) Hakuyū listed as dharma heir in the Soto lineage chart.
  - publisher: "Taizan Maezumi — Wikipedia"
    url: https://en.wikipedia.org/wiki/Taizan_Maezumi
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      Maezumi studied koans with Hakuun Yasutani and lay-teacher Koryū Osaka; he also received inka from Yamada Koun as part of his koan training, combining Sōtō-style shikantaza with Harada Daiun Sogaku's kōan-curriculum.
reducer_notes: |
  Taizan Maezumi received transmission from multiple teachers
  including Yamada Koun, Hakuun Yasutani (inka 1970), and Koryu Osaka (inka 1973). The
  Yamada Koun transmission is documented by the Terebess lineage chart which lists Maezumi
  as a dharma heir of Yamada Koun. This was part of Maezumi's koan training in the
  Sanbo-Zen/Sanbo Kyodan tradition. The White Plum Asanga whiteplum.org/founder/ page
  (the legacy source) also documents this relationship.
human_review_needed: false
---
