---
student: licun
teacher: linji-yixuan
tier: D
verified_at: "2026-05-22"
sources:
  - publisher: "Wikipedia — Linji Yixuan"
    url: https://en.wikipedia.org/wiki/Linji_Yixuan
    domain_class: reference
    retrieved_on: "2026-05-22"
    quote: |
      Linji's students included Zhaozhou Congshen, Puhua, Sansheng Huiran,
      Baoshou Yanzhao, Xinghua Cunjiang, Mayu, Longya Judun, Dajue, and
      Xingshan Jianhong. Of Linji's small body of disciples, only Xinghua
      Cunjiang's line of transmission survived.
  - publisher: "New World Encyclopedia — Lin-chi"
    url: https://www.newworldencyclopedia.org/entry/Lin-chi
    domain_class: reference
    retrieved_on: "2026-05-22"
    quote: |
      Linji left behind a small body of disciples, of which only Xinghua
      Cunjiang's line of transmission survived. Linji's twenty-odd spiritual
      successors are obscure and his lineage did not flourish initially.
  - publisher: "Encyclopedia of Buddhism — Linji zong"
    url: https://encyclopediaofbuddhism.org/wiki/Linji_zong
    domain_class: academic
    retrieved_on: "2026-05-22"
    quote: |
      Of Linji's small body of disciples, only Xinghua Cunjiang's line of
      transmission survived. Very little is known of Cunjiang's heir Nanyuan
      Huiyong. The three succeeding figures from Nanyuan were Fengxue Yanzhao,
      Shoushan Shengnian, and Fenyang Shanzhao.
reducer_notes: |
  Tier D confirmed after comprehensive search. The Jingde Chuandeng Lu
  (T2076) vol. 12 lists Linji Yixuan's dharma heirs under
  "鎮州臨濟義玄禪師法嗣二十二人" (twenty-two dharma heirs). The 22
  documented names are: 灌谿志閑、譚空、寶壽沼、三聖慧然、大覺、興化存獎、
  善崔、萬歲、雲山、桐峯菴主、杉洋菴主、涿州級衣、虎谿菴主、覆盆菴主、
  歷村、米倉、齊聳、涿州秀、善權徹、金沙、允誠、新羅智異山.

  Note: 歷村 (Licun, "village past") appears in the JDCDL list, but this is
  distinct from 力存 (Licun, "preserving strength") — the slug used in the
  lineage data. If the intended figure is 歷村 (the anonymous "Licun hermitage
  master" of the JDCDL), this is a completely anonymous figure with no
  biographical record. If it is 力存, that romanization does not match any
  of the 22 JDCDL names and is absent from Wikipedia, New World Encyclopedia,
  Encyclopedia of Buddhism, and Terebess/WWZC master lists.

  Three independent reference/academic sources confirm that Linji Yixuan's
  ~22 successors are "obscure" and that only Xinghua Cunjiang's line survived.
  No independent source positively identifies a named disciple "Licun"
  transmitting Linji's dharma in a form that constitutes a live lineage.

  2026-05-25 re-search: Terebess master.html and WWZC master list do not
  contain any entry for Licun. Wikipedia Linji Yixuan article does not list
  歷村. English and European-language web searches return no usable result
  for this anonymous figure. Tier D confirmed. Resolution requires primary-text
  pass of JDCDL T2076.12 via CBETA (cbetaonline.dila.edu.tw), which requires
  manual browsing of the Chinese text.
human_review_needed: true
---
