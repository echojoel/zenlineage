---
student: muzhou-daoming
teacher: huangbo-xiyun
tier: B
verified_at: 2026-05-15
sources:
  - publisher: "Terebess Asia Online — Muzhou Daoming"
    url: https://terebess.hu/zen/muzhou.html
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      Muzhou Daoming was a prominent disciple of Master Huangbo Xiyun, and served as the head monk at Huangbo's monastery in Jiangxi. Muzhou became the disciple, and eventually the heir, of Huangbo.
  - publisher: "Encyclopedia of Buddhism — Huangbo Xiyun"
    url: https://encyclopediaofbuddhism.org/wiki/Huangbo_Xiyun
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      Muzhou Daoming served as head monk under Huangbo. He played a crucial role in Linji's development by directing the young practitioner to seek direct engagement with the master.
  - publisher: "Wikipedia — Huangbo Xiyun"
    url: https://en.wikipedia.org/wiki/Huangbo_Xiyun
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      Linji Yixuan was directed by the head monk, Muzhou Daoming, to question Huangbo on the meaning of Buddhism. Before Huangbo died, he named thirteen successors.
reducer_notes: |
  Muzhou Daoming's status as Huangbo's dharma heir is attested by Terebess ("became the disciple, and eventually the heir, of Huangbo") and corroborated by multiple encyclopaedic sources confirming his role as head monk at Huangbo's monastery. Wikipedia notes Huangbo named thirteen successors without listing all of them; Muzhou is consistently identified as among those heirs. Two independent reference hosts (Terebess + EncBuddhism) with substantive quotes meets Tier B. No institutional source found; capped at B.
human_review_needed: false
---
