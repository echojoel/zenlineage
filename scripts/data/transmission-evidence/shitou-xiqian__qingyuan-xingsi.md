---
student: shitou-xiqian
teacher: qingyuan-xingsi
tier: B
verified_at: "2026-05-15"
sources:
  - publisher: "Wikipedia — Qingyuan Xingsi"
    url: https://en.wikipedia.org/wiki/Qingyuan_Xingsi
    domain_class: reference
    retrieved_on: "2026-05-15"
    quote: |
      Shitou's original teacher, Huineng, died when Shitou was only 13, so Qingyuan was necessary for him to receive legitimate dharma transmission.
  - publisher: "Encyclopedia of Buddhism — Shitou Xiqian"
    url: https://encyclopediaofbuddhism.org/wiki/Shitou_Xiqian
    domain_class: reference
    retrieved_on: "2026-05-15"
    quote: |
      After Huineng's death, Shitou Xiqian became a disciple of one of Huineng's main students, Qingyuan Xingsi… eventually made Xiqian his principal dharma heir.
  - publisher: "White Wind Zen Community — Master List of Masters"
    url: https://wwzc.org/master-list-masters/
    domain_class: sangha
    retrieved_on: "2026-05-15"
    quote: |
      He was accepted as a student by Huineng in 713. After Huineng died...he went to study with Qingyuan in Zhihzhou. He studied with Qingyuan for twelve years.
reducer_notes: |
  Three independent sources confirm Qingyuan
  Xingsi as Shitou Xiqian's dharma teacher: Wikipedia explaining the historical
  necessity of Qingyuan as teacher after Huineng's early death, Encyclopedia of
  Buddhism stating Qingyuan "made Xiqian his principal dharma heir," and WWZC
  with the detail of twelve years of study at Qingyuan's monastery. Upgraded
  from C to B: two independent reference hosts (en.wikipedia.org,
  encyclopediaofbuddhism.org) plus institutional sangha source (wwzc.org).
human_review_needed: false
---
