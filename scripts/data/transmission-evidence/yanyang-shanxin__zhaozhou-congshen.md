---
student: yanyang-shanxin
teacher: zhaozhou-congshen
tier: B
verified_at: 2026-05-15
sources:
  - publisher: "Terebess Asia Online — Master List"
    url: https://terebess.hu/english/master.html
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      "Yanyang Shanxin (Yen-yang Shan-hsin, Genyo Zenshin) (n.d.): One of
      two Dharma-heirs of Zhaozhou."
  - publisher: "Zhaozhou Congshen — Wikipedia"
    url: https://en.wikipedia.org/wiki/Zhaozhou_Congshen
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      "Zhaozhou's lineage died out quickly due to the many wars and frequent
      purges of Buddhism in China at the time, and cannot be documented
      beyond the year 1000."
reducer_notes: |
  Yanyang Shanxin is attested as one of Zhaozhou's
  two dharma heirs by the Terebess master list. The Zhaozhou Wikipedia article
  confirms his lineage was limited, consistent with only two documented heirs.
  Tier B: one reference-class source with explicit naming, one corroborating
  contextual source.
human_review_needed: false
---

Yanyang Shanxin is listed in the Terebess Asia Online master list as "One of
two Dharma-heirs of Zhaozhou." The Zhaozhou Wikipedia article confirms that his
lineage was very limited ("died out quickly"), consistent with only two recorded
successors. Yanyang Shanxin is the most documented of those two heirs in
English-language Chan resources.
