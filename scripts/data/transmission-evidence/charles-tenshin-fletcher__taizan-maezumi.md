---
student: charles-tenshin-fletcher
teacher: taizan-maezumi
tier: A
verified_at: 2026-05-15
sources:
  - publisher: "Yokoji Zen Mountain Center"
    url: https://zmc.org/tenshin
    domain_class: institutional
    retrieved_on: 2026-05-15
    quote: |
      In 1994, Tenshin Roshi received Dharma Transmission (authorization to teach) in the White Plum lineage from Maezumi Roshi
  - publisher: "White Plum Asanga"
    url: https://whiteplum.org/founder/
    domain_class: institutional
    retrieved_on: 2026-05-15
    quote: |
      He transmitted the Dharma to twelve successors: Bernard Tetsugen Glassman, Dennis Genpo Merzel, Charlotte Joko Beck, Jan Chozen Bays, John Daido Loori, Gerry Shishin Wick, John Tesshin Sanderson, Alfred Jitsudo Ancheta, Charles Tenshin Fletcher, Susan Myoyu Andersen, Nicolee Jikyo Miller, and William Nyogen Yeo.
  - publisher: "Wikipedia (Charles Tenshin Fletcher)"
    url: https://en.wikipedia.org/wiki/Charles_Tenshin_Fletcher
    domain_class: reference
    retrieved_on: 2026-05-15
    quote: |
      In 1994, he received Dharma transmission (authorization to teach) in the White Plum lineage from Taizan Maezumi Rōshi.
reducer_notes: |
  R2: confidence=high
  R3: confidence=high
human_review_needed: false
---
