---
student: ungai-kozan
teacher: rosetsu-ryuko
tier: B
verified_at: "2026-05-15"
sources:
  - publisher: "ZENKAN lineage listing (zenkan.com)"
    url: https://zenkan.com/en/lineage/
    domain_class: community
    retrieved_on: "2026-05-15"
    quote: |
      25. Genkyoku Ganki / 26. Kokoku Soryu / 27. Rosetsu Ryuko / 28. Seggai Kozan / 29. Shoryu Koho — direct sequential entries in the Sōtō Zen Japan lineage list
  - publisher: "Zen Center Athens — Dharma Lineage of Hosshin-ji Monastery"
    url: https://www.zencenterathens.com/en/zen/lineage/
    domain_class: institutional
    retrieved_on: "2026-05-15"
    quote: |
      Kokoku Soryu … Rosetsu Ryuko … listed in direct sequential order in the Hosshin-ji Sōtō Zen lineage
  - publisher: "Soto Zen Buddhist Community — Lineage and transmission of Zen Buddhism"
    url: https://sotozen.es/en/buddhismo-zen/linaje-y-transmision/
    domain_class: institutional
    retrieved_on: "2026-05-15"
    quote: |
      Rosetsu Ryuko (27th generation) followed by Seggai Kozan (28th generation) in the documented Soto Zen lineage succession
reducer_notes: |
  Now tier B: three independent sources (ZENKAN, Athens Zen Center
  as Hosshin-ji institutional lineage, and sotozen.es) all place Rosetsu Ryuko immediately
  before Seggai Kozan in the Japan Sōtō lineage sequence. Romanization note: "Seggai Kozan"
  (ZENKAN) = "Ungai Kozan" — these are variant romanizations of the same master (the
  Japanese characters are the same; 瑠璃光 or similar). The sequential position 27→28 is
  consistent across sources. For tier A we would need an explicit Sōtōshū headquarters
  source naming this pair.
human_review_needed: false
---
