...
export function parseHtml(html: string, sourceId: string, ingestionRunId: string): RawMaster[] {
  ...
  $("table.wikitable").each((_tableIdx, table) => {
    ...
    const colIdx = {
      ...
      cjk: headers.findIndex(
        (h) => h.includes("chinese") || h.includes("cjk") || h.includes("kanji")
      ),
...
async function main() {
  ...
  const run = await startIngestionRun({
    sourceId,
    scriptName: "extract-wikipedia.ts",
  });
  ...
  try {
    ...
    await finishIngestionRun(run, {
      ...
      notes: `Extracted ${masters.length} masters from Wikipedia lineage HTML.`,
...
const isDirectRun =
  process.argv[1]?.endsWith("extract-wikipedia.ts") ||
