...
describe('reconcile — teacher reference resolution produces transmissions', () => {
  it('creates a transmission edge when teacher resolves to a canonical master', () => {
    ...
    const dongshan = result.masters.find(
      (m) => m.names[0].value === 'Dongshan Liangjie',
    );
    expect(dongshan).toBeDefined();

    const tx = result.transmissions.find(
      (t) => t.teacher_id === dongshan!.id,
    );
...
describe('deterministicId', () => {
  it('returns the same ID for the same seed', () => {
    expect(deterministicId('master:bodhidharma')).toBe(
      deterministicId('master:bodhidharma'),
    );
  });

  it('returns different IDs for different seeds', () => {
    expect(deterministicId('master:bodhidharma')).not.toBe(
      deterministicId('master:huike'),
    );
  });
